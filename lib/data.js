...
function saveItems(items, callback) {
  ...
  connection.connect(function connectionAttempted(err) {
    if (err) {
      ...
      var sql = 'REPLACE INTO activities (happened_on, github_organization, github_repository, github_username, github_public_email, github_commit_url, action_type, commit_id, commit_msg) VALUES ?';
      var values = items;
...
function getOldestOrNewestCommitDate (repo, oldestOrNewest, callback) {
  var connection = mysql.createConnection(connectionOptions);
  connection.connect(function connectionAttempted(err) {
    if (err) {
      console.error(err);
      ...
      if (oldestOrNewest === 'oldest') {
        sql = 'SELECT * FROM gitribution2.activities WHERE github_organization=? AND github_repository=? ORDER BY happened_on asc limit 1;';
      } else {
        sql = 'SELECT * FROM gitribution2.activities WHERE github_organization=? AND github_repository=? ORDER BY happened_on desc limit 1;';
      }
...
function getOldestCommitDate(repo, callback) {
  getOldestOrNewestCommitDate(repo, 'oldest', function (err, res) {
    callback(res);
  });
}
...
function getLatestCommitDate(repo, callback) {
  getOldestOrNewestCommitDate(repo, 'newest', function (err, res) {
    callback(res);
  });
}
...
function getActiveContributors(date, team, callback) {
  console.log('getActiveContributors', date, team);
  var connection = mysql.createConnection(connectionOptions);
  connection.connect(function connectionAttempted(err) {
    if (err) {
      console.error(err);
...
function update2014TotalActive(team, callback) {
  console.log('update2014TotalActive');
  // timer to check impact of loading
  console.time('getData');
