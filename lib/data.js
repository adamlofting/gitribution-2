  ...
  connection.connect(function connectionAttempted(err) {
    if (err) {
      ...
      if (oldestOrNewest === 'oldest') {
        sql = 'SELECT * FROM activities WHERE github_organization=? AND github_repository=? ORDER BY happened_on asc limit 1;';
      } else {
        sql = 'SELECT * FROM activities WHERE github_organization=? AND github_repository=? ORDER BY happened_on desc limit 1;';
      }
