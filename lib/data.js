...
function getOldestOrNewestCommitDate(repo, oldestOrNewest, actionType, callback) {
  var connection = mysql.createConnection(connectionOptions);
  connection.connect(function connectionAttempted(err) {
    if (err) {
      console.error(err);
...
function getOldestCommitDate(repo, callback) {
  getOldestOrNewestCommitDate(repo, 'oldest', 'commit-author', function (err, res) {
    callback(null, res);
  });
}
...
function getLatestCommitDate(repo, callback) {
  getOldestOrNewestCommitDate(repo, 'newest', 'commit-author', function (err, res) {
    callback(null, res);
  });
}
...
function getOldestPullRequestDate(repo, callback) {
  getOldestOrNewestCommitDate(repo, 'oldest', 'pull-request-opened', function (err, res) {
    callback(null, res);
  });
}
...
function getLatestPullRequestDate(repo, callback) {
  getOldestOrNewestCommitDate(repo, 'newest', 'pull-request-opened', function (err, res) {
    callback(null, res);
  });
}
