 ...
 */
function saveItem(happenedOn, githubOrgName, githubRepo, githubLogin, githubPublicEmail, actionType, htmlUrl, callback) {
...
function saveItems(items, callback) {
  ...
  connection.connect(function connectionAttempted(err) {
    if (err) {
      ...
      var sql = 'REPLACE INTO activities (happened_on, github_organization, github_repository, github_username, github_public_email, contribution_url, action_type) VALUES ?';
      var values = items;
