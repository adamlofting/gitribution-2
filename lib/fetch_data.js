...
function getEmailForLogin(login, callback) {
  ...
  var cacheKey = 'login' + login;
  var cache = myCache.get(cacheKey);
  if (cache[cacheKey]) {
    return callback(null, cache[cacheKey]);
  }

  // if this isn't in the cache, we check with github
  var githubClient = createGithubClient();
  ...
  githubClient.user.getFrom(msg, function gotFromRepo(err, res) {
    ...
    if (res && res.email) {
      myCache.set(cacheKey, res.email, 600);
  ...
  if (ghRes) {
    async.eachLimit(ghRes, 4,
      ...
      function checkedAll (err) {
        if (toSave.length > 0) {
          data.saveItems(toSave, function saved(err) {
            console.log('Saved', toSave.length, 'pull-requests for:', repo.org, repo.name);
    ...
    function waterfallCommitsHistoric(callback) {
      data.getOldestCommitDate(repo, function gotOldestCommitDate(err, oldestDateInOurDB) {
        ...
        getCommitsForRepoUntil(repo, oldestDateInOurDB, OLDEST_DATE_WE_CARE_ABOUT, function gotNewCommits(err) {
          console.log('Commits: updated historic:', repo.org, repo.name);
          callback(null);
        });
      });
    },
    function waterfallCommitsNew(callback) {
      data.getLatestCommitDate(repo, function gotLatestCommitDate(err, latestDateInOurDB) {
        ...
        getCommitsForRepoUntil(repo, now, latestDateInOurDB, function gotNewCommits(err) {
          console.log('Commits: updated recent:', repo.org, repo.name);
          callback(null);
        });
      });
    },
...
function workOnRepoPullRequests(repo, callback) {
  ...
  async.waterfall([
    ...
    function waterfallPRsHistoric(callback) {
      data.getOldestPullRequestDate(repo, function gotOldestPRDate(err, oldestDateInOurDB) {
        ...
        getPullRequestsForRepoUntil(repo, until, OLDEST_DATE_WE_CARE_ABOUT, function gotNewCommits(err) {
          console.log('Pull Requests: updated historic:', repo.org, repo.name);
          callback(null);
        });
      });
    },
    function waterfallPRsNew(callback) {
      data.getLatestPullRequestDate(repo, function gotLatestPRDate(err, latestDateInOurDB) {
        // because this function runs after the one above, there will always be existing data
        // unless a repo has no PRs in which case we should skip this fetch
        if (!latestDateInOurDB) {
          return callback(null);
        ...
        // use the latest date for another pass over the API
        getPullRequestsForRepoUntil(repo, now, latestDateInOurDB, function gotNewCommits(err) {
          console.log('Pull Requests: updated recent:', repo.org, repo.name);
          callback(null);
        });
      });
    }
 ...
 */
exports.fetchMoreCommitData = function fetchMoreCommitData(callback) {
        ...
        function waterfallWorkOnRepos(repos, callback) {
          async.eachLimit(repos, 3,
            function eachRepo(repo, callback) {
              console.log('Working on Commits for:', repo.org, repo.name);
 ...
 */
exports.fetchMorePullRequestData = function fetchMorePullRequestData(callback) {
  // loads the github org names listed in to_track.js
  async.eachLimit(toTrack.orgs, 2,
    // for each github org we care about
    function workOnThisOrg(org, callback) {
      ...
      async.waterfall([
        ...
        function waterfallWorkOnRepos(repos, callback) {
          async.eachLimit(repos, 3,
            function eachRepo(repo, callback) {
              console.log('Working on PRs for:', repo.org, repo.name);
